...
class controller:
    ...
    def backoff_control(self, total_load, rho, p_d, K, Z, MODE,n):
        ...
        self.p_b, self.observe_pi = backoff_control.backoff_control(N_tilde, self.p_b, rho, self.Dmax, p_d, K, Z, MODE, total_load)
        if n % 10 == 0:
...
def main(RHO, NUM_SAT, SECONDS, NUM_UE,MODE, SEED, NUM_EPOCHS):
    ...
    print(f"Mode: {MODE}, Active rate: {RHO}, Satellite orbits: {NUM_SAT}, Time Slots: {SECONDS}")
    # 取得真實衛星列表 (Top-N 軌道面)
    # 設定模擬開始時間
    ts = load.timescale()
